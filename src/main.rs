...
fn main() {
    ...
    for username in config.monitoring_username.iter() {
        ...
        let existing_followings = Some(vec!["733610607249776641".to_string()]);
        let following_query = FollowingFetcher::new(&fetched_user.user.id, existing_followings);
        ...
        //let (query_result, _) = following_query.fetch(&config).expect("error in query following");
